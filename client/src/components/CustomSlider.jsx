...
export default function CustomSlider({ value, max, onChange, showThumb = true }) {
  ...
  return (
    <div
      ...
      className="group relative font-montserrat tracking-wide cursor-pointer"
      style={{
        height: "1.2rem",
        display: "flex",
        alignItems: "center"
    ...
    >
      ...
      <div
        className="w-125 bg-white/20 rounded-full relative transition-[height] duration-200"
        // className="w-full bg-black/20 rounded-full relative transition-[height] duration-200"
        style={{
      ...
      >
        ...
        {showThumb && (
          <div
            ...
            // className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-1 h-3 bg-black rounded-full shadow-lg 
            //            ${isDragging ? "opacity-100 scale-100" : "opacity-0 scale-0 group-hover:opacity-100 group-hover:scale-100"} 
            //            transition-[opacity,transform] duration-200`}
            style={{
              left: `${(value / max) * 100}%`,
